...
#define __ARM_DEFINES_H__
    ...
    #endif
    #define TARGET_PAGESIZE (PAGESIZE-1)
  #endif
  #ifndef TARGET_ALLOCSIZE
    #define TARGET_ALLOCSIZE (TARGET_PAGESIZE+1)
  #endif
#else
  #ifndef TARGET_PAGESIZE
    #define TARGET_PAGESIZE 0
  #endif
  #ifndef TARGET_ALLOCSIZE
    #define TARGET_ALLOCSIZE 0
  #endif
#endif

//#define WITH_DISASM 1
...
#define TEMP_1 			ARMREG_R2
#define TEMP_2 			ARMREG_R3
...
{										                        \
	  /*ARM_PUSH(ARM_POINTER, SAVEREG_MASK);*/ /* Optimize? */        \
    LoadImmediate32(((u32)func), TEMP_2);   \
    ARM_BLX(ARM_POINTER, TEMP_2);            \
		/*ARM_BL(ARM_POINTER, arm_relative_offset(recMem, func, 8));*/	\
  	/*ARM_POP(ARM_POINTER, SAVEREG_MASK);*/ /* Optimize? */         \
}										                        \
...
inline void LoadImmediate32(u32 imm, u32 reg)
{
	if( imm )		
	{
		...
		if(imm & 0xF0000000 && imm & 0x0000000F)
		{
...
{										                                                \
  u32 store_count = arm_disect_imm_32bit(imm32, stores, rotations);	\
  u32 i;									                                          \
										                                                \
  if(store_count > 2)                                               \
  {                                                                 \
    ARM_MOVW_REG_IMM(0, reg, imm32&0x0000FFFF);                     \
    ARM_MOVT_REG_IMM(0, reg, imm32&0xFFFF0000);                     \
  }                                                                 \
  else                                                              \
  {                                                                 \
