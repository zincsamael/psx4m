...
extern void set_cpsr(u32 flags);
extern void set_bx(u32 addr);
...
{
	CLEAR_INSN_CACHE(recMemBase, RECMEM_SIZE);
}
...
static u32 recRecompile()
{
  ...
  if (mprotect(recMemBase, RECMEM_SIZE, PROT_READ|PROT_WRITE))
    ...
    ARM_MOV_REG_IMM(ARM_POINTER, ARMREG_R0, (u32)recMemStart, 0);
    CALLFunc((u32)tester);
	...
	if( isInBios )
	{
		...
		else if( isInBios == 2 && psxRegs->pc == 0x80030000 )
		{
			...
			CLEAR_INSN_CACHE(recMemStart, (u32)recMem - (u32)recMemStart);
    fprintf(stderr, "recmemstart 0x%x \n", recMemStart);
#if 0 //def IPHONE
...
#ifdef IPHONE  
  recMemBase = (unsigned char *)(((unsigned long) recMemBaseStatic + TARGET_PAGESIZE) & ~(TARGET_PAGESIZE));
  if (mprotect(recMemBase, RECMEM_SIZE, PROT_READ|PROT_WRITE|PROT_EXEC))
  	fprintf(stderr, "Couldn't mprotect ram!");
  memset(recMemBase, 0, RECMEM_SIZE);
  /*
