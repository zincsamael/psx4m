...
#define execI() { 						\
	...
	debugI(); 						\
 								\
	psxRegs->pc+= 4; psxRegs->cycle += (u32)BIAS_CYCLE_INC; 			\
...
//	SysPrintf("delayRead at %x!\n", psxRegs->pc);
	...
	psxRegs->cycle += (u32)BIAS_CYCLE_INC; //(int)((float)((psxRegs->pc-oldpc)/4) * BIAS_CYCLE_INC);
	psxRegs->pc = bpc;
	pc = oldpc = psxRegs->pc;
...
void psxDelayTest(int reg, u32 bpc)
{
	...
	psxRegs->cycle += (u32)BIAS_CYCLE_INC;
	psxRegs->pc = bpc;
	pc = oldpc = psxRegs->pc;
