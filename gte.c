...
void gteMFC2(u32 code) {
	...
	psxRegs->GPR.r[_fRt_(code)] = MFC2(_fRd_(code));
}

void gteCFC2(u32 code) {
	/* Misc helper */
	if (!_fRt_(code)) return;
	psxRegs->GPR.r[_fRt_(code)] = psxRegs->CP2C.r[_fRd_(code)];
}

void gteMTC2(u32 code) {
	/* Misc helper */
	MTC2(psxRegs->GPR.r[_fRt_(code)], _fRd_(code));
}

void gteCTC2(u32 code) {
	/* Misc helper */
	psxRegs->CP2C.r[_fRd_(code)] = psxRegs->GPR.r[_fRt_(code)];
}
